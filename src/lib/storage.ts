...
function rowToInvitation(row: typeof invitations.$inferSelect): Invitation {
  return {
    ...
    akadVenue: row.akadVenue ?? undefined,
    akadTime: row.akadTime ?? undefined,
...
export async function saveInvitation(invitation: Invitation): Promise<void> {
  await db
    ...
    .values({
      ...
      akadVenue: invitation.akadVenue ?? null,
      akadTime: invitation.akadTime ?? null,
      googleMapsUrl: invitation.googleMapsUrl ?? null,
